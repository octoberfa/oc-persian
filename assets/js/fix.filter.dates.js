...
+ function ($) {
    ...
    FilterWidget.prototype.initDatePickers = function (isRange) {
        ...
        $inputs.each(function (index, datepicker) {
            ...
            var $datepicker = $(datepicker),
                defaults = {
                    minDate: (new Date(scopeData.minDate)).getTime(),
                    maxDate: (new Date(scopeData.maxDate)).getTime(),
            ...
            $datepicker.data('pikaday', $datepicker.pDatepicker(defaults))
        })
    }
    ...
    FilterWidget.prototype.updateScopeDateSetting = function ($scope, dates) {
        ...
        var $setting = $scope.find('.filter-setting'),
            ...
            jDateFormat = this.getjDateFormat(),
        ...
        if (dates && dates.length) {
            ...
            if (dates.length > 1) {
                ...
                if (dates[0] || dates[1]) {
                    var after = dates[0] ? moment(moment.tz(dates[0], this.appTimezone).tz(this.timezone)).format(jDateFormat) : '∞',
                        before = dates[1] ? moment(moment.tz(dates[1], this.appTimezone).tz(this.timezone)).format(jDateFormat) : '∞'
                ...
                $setting.text(moment(moment.tz(dates[0], this.appTimezone).tz(this.timezone)).format(jDateFormat))
            } else {
                reset = true
            }
        } else {
    ...
    FilterWidget.prototype.filterByDate = function (isReset) {
        ...
        if (!isReset) {
            ...
            datepickers.each(function (index, datepicker) {
                var date = $(datepicker).data('pikaday').getState().selected
                if (date) {
                    date = moment(date.dateObject.toDate()).format('YYYY-MM-DD')
                }
    ...
    FilterWidget.prototype.getDateFormat = function () {
        if (this.locale) {
            return moment()
                .locale(this.locale)
                .localeData()
                .longDateFormat('l').replace(/j/g, '')
        }

        return 'YYYY-MM-DD'
    }
    FilterWidget.prototype.getjDateFormat = function () {
        if (this.locale) {
            return moment()
                .locale(this.locale)
                .localeData()
    ...
    FilterWidget.prototype.onCheckDocumentClickTargetDatePicker = function (target) {
        ...
        return $target.parents('.datepicker-plot-area').length
        // hasClass('pwt-btn') ||
        //     $target.hasClass('header-row-cell') ||
        //     $target.hasClass('other-month') ||

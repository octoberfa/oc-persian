...
+function ($) {
    ...
    FilterWidget.prototype.initFilterDate = function () {
        ...
        $(document).on('click', '#controlFilterPopoverDateJalali [data-trigger="filter"]', function (e) {
            e.preventDefault()
            e.stopPropagation()
            self.filterByDate()
        })

        $(document).on('click', '#controlFilterPopoverDateJalali [data-trigger="clear"]', function (e) {
            e.preventDefault()
            e.stopPropagation()
    ...
    FilterWidget.prototype.getPopoverDateTemplate = function () {
        return '                                                                                                        \
                <form>                                                                                                  \
                    ...
                    <div id="controlFilterPopoverDateJalali" class="control-filter-popover control-filter-box-popover">       \
    ...
    FilterWidget.prototype.getPopoverRangeTemplate = function () {
        return '                                                                                                          \
                <form>                                                                                                    \
                    <input type="hidden" name="scopeName" value="{{ scopeName }}" />                                      \
                    <div id="controlFilterPopoverDateJalali" class="control-filter-popover control-filter-box-popover --range"> \
                        <div class="filter-search loading-indicator-container size-input-text">                           \
                            <div class="field-datepicker">                                                                \
                                <div class="input-with-icon right-align">                                                 \
                                    <i class="icon icon-calendar-o"></i>                                                  \
    ...
    FilterWidget.prototype.initDatePickers = function (isRange) {
        var self = this,
            ...
            $inputs = $('.field-datepicker input', '#controlFilterPopoverDateJalali'),
        ...
        $inputs.each(function (index, datepicker) {
            ...
            if (0 <= index && index < data.dates.length) {
                defaultValue = data.dates[index] ? moment(moment.tz(data.dates[index], self.appTimezone).tz(self.timezone)) : ''
            }
    ...
    FilterWidget.prototype.clearDatePickers = function () {
        var $inputs = $('.field-datepicker input', '#controlFilterPopoverDateJalali')
    ...
    FilterWidget.prototype.updateScopeDateSetting = function ($scope, dates) {
        ...
        if (dates && dates.length) {
            ...
            if (dates.length > 1) {
                ...
                if(dates[0] || dates[1]) {
                    var after = dates[0] ? moment(moment.tz(dates[0], this.appTimezone).tz(this.timezone)).format(dateFormat) : '∞',
                        before = dates[1] ? moment(moment.tz(dates[1], this.appTimezone).tz(this.timezone)).format(dateFormat) : '∞'
    ...
    FilterWidget.prototype.filterByDate = function (isReset) {
        ...
        if (!isReset) {
            var datepickers = $('.field-datepicker input', '#controlFilterPopoverDateJalali')
